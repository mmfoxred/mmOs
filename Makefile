...
#	$(shell rm -rf $(BUILD)/$(HD_IMG_NAME))
	...
	dd if=${BUILD}/boot/boot.o of=$(BUILD)/$(HD_IMG_NAME) bs=446 seek=0 count=1 conv=notrunc
	dd if=${BUILD}/boot/setup.o of=$(BUILD)/$(HD_IMG_NAME) bs=512 seek=3450 count=2 conv=notrunc
	dd if=${BUILD}/system.bin of=$(BUILD)/$(HD_IMG_NAME) bs=512 seek=3452 count=60 conv=notrunc
...
${BUILD}/kernel.bin: ${BUILD}/boot/head.o ${BUILD}/init/main.o ${BUILD}/kernel/asm/io.o ${BUILD}/kernel/chr_drv/console.o \
	...
	${BUILD}/kernel/kernel_thread.o ${BUILD}/kernel/assert.o ${BUILD}/kernel/ya_shell.o ${BUILD}/kernel/blk_drv/hd.o ${BUILD}/kernel/asm/hd_handler.o \
	${BUILD}/kernel/blk_drv/hd_lba28.o ${BUILD}/fs/buffer.o ${BUILD}/kernel/blk_drv/ll_rw_blk.o ${BUILD}/kernel/blk_drv/fat32/driver.o \
	${BUILD}/kernel/blk_drv/fat32/xdisk.o ${BUILD}/kernel/blk_drv/fat32/xfat.o  ${BUILD}/kernel/blk_drv/fat32/xfat_buf.o ${BUILD}/kernel/blk_drv/fat32/xfat_obj.o
...
${BUILD}/kernel/blk_drv/fat32/%.o: oskernel/kernel/blk_drv/fat32/%.c
	$(shell mkdir -p ${BUILD}/kernel/blk_drv/fat32)
	gcc ${CFLAGS} ${DEBUG} -c $< -o $@
