...
xfat_err_t diritem_init_default(diritem_t *dir_item, xdisk_t *disk, u8_t is_dir, const char *name, u32_t cluster);

xfat_err_t destroy_cluster_chain(xfat_t *xfat, u32_t cluster);
...
static u32_t cal_fat_tbl_sectors(dbr_t *dbr, xdisk_part_t *xdisk_part, xfat_fmt_ctrl_t *ctrl) {
    ...
    u32_t fat_sector_count =
            fat_dat_sectors / (dbr->bpb.BPB_NumFATs + sector_size * dbr->bpb.BPB_SecPerClus / sizeof(cluster32_t));
...
static xfat_err_t create_dbr(xdisk_part_t *xdisk_part, xfat_fmt_ctrl_t *ctrl, xfat_fmt_info_t *fmt_info) {
    ...
    strcpy((char *) dbr->bpb.BS_OEMName, "XFAT SYS");
...
static xfat_err_t allocate_free_cluster(xfat_t *xfat, u32_t curr_cluster, u32_t count,
                                        u32_t *r_start_cluster, u32_t *r_allocated_count, u8_t en_erase,
                                        u8_t erase_data) {
    xfat_err_t err;
...
static xfat_err_t locate_file_dir_item(xfat_t *xfat, u8_t locate_type, u32_t *dir_cluster, u32_t *cluster_offset,
    ...
    xdisk_t *xdisk = xfat_get_disk(xfat);
    u32_t initial_sector = to_sector(xdisk, *cluster_offset);         //簇中偏移，是指向的扇区相对簇起始的偏移
    u32_t initial_offset = to_sector_offset(xdisk, *cluster_offset); //好像是扇区里的偏移
    ...
    if ((path != 0) && (*path != '\0')) {
        ...
        while (curr_path != (const char *) 0) {
            ...
            xfat_err_t err = locate_file_dir_item(xfat, XFILE_LOCATE_DOT | XFILE_LOCATE_NORMAL,
                                                  &parent_cluster, &parent_cluster_offset, curr_path, &moved_bytes,
                                                  &dir_item);
            if (err < 0) {
                return err;
            }
            ...
            //这里的path是 a/b/c/d.txt 这样的，是后半部分
...
xfat_err_t xdir_next_file(xfile_t *file, xfileinfo_t *info) {
    ...
    // 搜索文件或目录
    //接着从上一个文件的同一个簇后面开始
    //这里算偏移 算的是 ((pos) % ((xfat)->cluster_byte_size))，即同一个簇的字节偏移数
 ...
 */
xfat_err_t diritem_init_default(diritem_t *dir_item, xdisk_t *disk, u8_t is_dir, const char *name, u32_t cluster) {
    xfile_time_t timeinfo = {.year = 2000,.month = 12,.day = 13,.hour = 15,.minute = 30,.second = 20};
    ...
    for (curr_path = path; curr_path != '\0'; curr_path = get_child_path(curr_path)) {
        do {
            xfat_err_t err = get_next_diritem(xfat, DIRITEM_GET_USED, curr_cluster, curr_offset,
                                              &found_cluster, &found_offset, &next_cluster, &next_offset, &buf,
                                              &diritem);
            if (err < 0) {
                return err;
            }
